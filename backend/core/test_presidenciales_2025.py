...
class TestSeedPresidenciales2025:
    def test_crea_tipo_eleccion_con_anio(self, datos_pesados):
        tipo = TipoEleccion.objects.get(nombre="Presidencial 2025")
        assert tipo.anio == 2025
        assert not tipo.es_base

    def test_crea_los_8_candidatos_oficiales(self, datos_pesados):
        tipo = TipoEleccion.objects.get(nombre="Presidencial 2025")
    ...
    def test_kast_puede_ser_multi_eleccion(self, datos_pesados):
        """Kast corre en 2025 (y quizas tambien en 2021)."""
...
class TestAnioTipoEleccion:
    def test_campo_anio_acepta_valores(self):
        tipo = TipoEleccion.objects.create(nombre="Test 2027", anio=2027)
        assert tipo.anio == 2027

    def test_puede_filtrar_por_anio(self):
        TipoEleccion.objects.create(nombre="Presi 2021 t", anio=2021)
        TipoEleccion.objects.create(nombre="Presi 2025 t2", anio=2025)
        TipoEleccion.objects.create(nombre="Alcaldes 2024 t", anio=2024)

        del_2025 = TipoEleccion.objects.filter(anio=2025, nombre="Presi 2025 t2")
